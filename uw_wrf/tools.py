...
def load_ensemble_xr(wrfensdir, memberdirs, dom=1, chunks=None, verbose=False):
    """
    ...
    mem_xrs = [load_member_xr('{}/{}'.format(wrfensdir, mem), dom=dom, verbose=verbose) for mem in memberdirs]
...
def load_member_xr(wrfoutdir, dom=1, verbose=False):
    ...
    wrfouts = '{}/wrfout_d{:01d}.*'.format(wrfoutdir, dom)
