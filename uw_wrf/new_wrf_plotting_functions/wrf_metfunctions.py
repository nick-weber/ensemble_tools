...
def plev_interp(VAR, PB, P, plev):
    ...
    aboveP = np.log(ptot.reshape(nz,ny*nx)[above.flatten(),range(ny*nx)].reshape(ny,nx))
    belowP = np.log(ptot.reshape(nz,ny*nx)[below.flatten(),range(ny*nx)].reshape(ny,nx))
    ...
    aboveV = varflat[above.flatten(),range(ny*nx)].reshape(ny,nx)
    belowV = varflat[below.flatten(),range(ny*nx)].reshape(ny,nx)
...
def melting_level(T, PB, P, PHB, PH, TBASE=300.0):
    ...
    aboveT = T.reshape(nz,ny*nx)[above.flatten(),range(ny*nx)].reshape(ny,nx)
    belowT = T.reshape(nz,ny*nx)[below.flatten(),range(ny*nx)].reshape(ny,nx)
...
def plev_height(PHB, PH, PB, P, PSFC, HGT, T, QVAPOR, plev, TBASE=300.0):
    ...
    if np.sum(locs,axis=None) != 0:
        ...
        nz,ny,nx = T.shape
        Tupper = T.reshape(nz,ny*nx)[vert_levs.flatten(),range(ny*nx)].reshape(ny,nx)[locs]
        Pupper = ptot.reshape(nz,ny*nx)[vert_levs.flatten(),range(ny*nx)].reshape(ny,nx)[locs]
...
### ADDED BY NICK WEBER ###
def calculate_preciprate(RAINNC, RAINC=None, t_axis=1):
    if RAINC is None:
        RAINT = RAINNC
    else:
        RAINT = RAINC + RAINNC
    prate = RAINT - np.roll(RAINT, 1, axis=t_axis)
    if t_axis==0:
        prate[0,:,:] = 0.
    elif t_axis==1:
        prate[:,0,:,:] = 0.
    else:
        raise ValueError('Unexpected "t_axis" value: {}'.format(t_axis))
    return prate  # mm/h
